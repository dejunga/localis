...
async function sendSMinimalnimTrajanjem(
  prev: RegistrationState,
  formData: FormData,
): Promise<RegistrationState> {
  const pocetak = Date.now();
  const rezultat = await sendSeminarRegistration(prev, formData);
  // Greške u poljima korisnik treba vidjeti odmah, bez čekanja
    ...
    <div className="fixed inset-0 z-60 flex items-center justify-center bg-navy/40 backdrop-blur-sm px-4 animate-in fade-in duration-200">
      ...
      >
        <h3 className="text-lg font-semibold text-[var(--navy)]">
          {imaPonudu ? "Obrađujemo vašu prijavu" : "Šaljemo vašu prijavu"}
        </h3>
...
export default function RegistracijaForm({
  ...
  const [state, formAction, pending] = useActionState(sendSMinimalnimTrajanjem, initialState);
